...
class RNN(object):
	...
	def predict(self, x):
		'''
		Forward prediction in my RNN
		predict an output sequence y for a given input sequence x
		
		x	list of words, as indices, e.g.: [0, 4, 2]
		
		returns	y,s--produced by concatenating hidden vectors s[t] and output vectors y[t] for t=1,2,...,n
		y	matrix of probability vectors for each input word
		s	matrix of hidden layers for each input word
		For a given input[w1,w2,...,wt], the probability of the next word at time step t+1 can be read from the output vector y[t]
		'''
		...
		for t in range(len(x)):
			...
			# --- your code here --- #
			##########################
			#x_t is the one-hot vector representing the vocabulary index of the word w_t
			x_t = np.zeros((1, self.vocab_size))
			x_t[0][x[t]] = 1
			#s:hidden layer/context layer/state
			#s[t],y[t]:hidden and output vectors produced after applying the sigmoid and softmax nonlinearities
			s[t] = sigmoid(self.V.dot(x_t.T) + np.expand_dims(self.U.dot(s[t - 1, :].T), axis=1)).T
			y[t] = softmax(self.W.dot(s[t])).T
		return y, s
	
	def acc_deltas(self, x, d, y, s):
		'''
		accumulate updates for V, W, U(The parameters to be learned)
		...
		for t in reversed(range(len(x))):
			...
			# --- your code here --- #
			##########################
			#d[t]=[d1_t,d2_t,...,dv_t] is the one-hot vector representing the vocabulary index of desired output word at time t.
			d_onehot = np.zeros(self.vocab_size)
			d_onehot[d[t]] = 1
			...
			#accumulate the weight updates for U,V and W for a simple backpropagation through the RNN.
		...
		for t in reversed(range(len(x))):
			...
			for tau in range(1, steps + 1):
				if t >= tau:
					...
					delta_in = self.U.T.dot(delta_in_previous) * s[t - tau] * (np.ones(self.hidden_dims) - s[t - tau])
	...
	def compute_loss(self, x, d):
		...
		return loss		the combined loss for all words--return the total loss produced by the model's predictions for the sentence.
		'''
		...
		d_onehot = np.zeros((len(d), self.vocab_size))
		d_onehot[range(len(d)), d] = 1
		#predict an output sequence y for a given input sequence x
		y_hat = self.predict(x)
        # return cross-entrophy loss
		loss = - np.sum(d_onehot * np.log(y_hat[0]))
		return loss
	...
	def compute_loss_np(self, x, d):
		...
		loss = - np.sum(d_onehot * np.log(y_hat[0][-1]))
		return loss
	...
	def compute_mean_loss(self, X, D):
		...
		for i in range(len(X)):
			...
			count_words += len(X[i])
		#compute the average loss over a corpus of input sentences. It should average across all words in all sentences of the given corpus
		mean_loss = total_loss / float(count_words)
		return mean_loss
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		print("best observed loss was {0}, at epoch {1}".format(run_loss, (best_epoch+1)))
	...
	def train_np(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		if min_change_count <= 2:
			print("\n\ntraining finished after reaching maximum of {0} epochs".format(epochs))
		print("best observed loss was {0}, acc {1}, at epoch {2}".format(run_loss, best_acc, (best_epoch+1)))
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		run_loss = my_rnn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback)
