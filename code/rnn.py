...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            ...
            # --- your code here --- #
            ##########################
            x_oh = np.zeros((self.vocab_size, 1))
            x_oh[x[t]] = 1
            net_in = np.dot(self.U, s[t - 1][:, None]) + np.dot(self.V, x_oh)
            s_t = sigmoid(net_in)
            s[t, :] = s_t.reshape(-1)
            net_out = np.dot(self.W, s_t)
            y[t, :] = softmax(net_out).reshape(-1)
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            # delta_out = np.eye(self.out_vocab_size)[:, d[t]:d[t]+1] - np.array(y[t])[:, None]
            # self.deltaW += delta_out @ s[t][None, :]
            # delta_in = self.W.T @ delta_out * ((s[t] * (1 - s[t]))[:, None])
            # self.deltaV += delta_in @ np.eye(self.vocab_size)[x[t]:x[t]+1, :]
            # self.deltaU += delta_in @ (s[t][None,:])
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in reversed(range(len(x))):
            ...
            # --- your code here --- #
            ##########################
            # delta_out = np.eye(self.out_vocab_size)[d[t]][:, None] - np.array(y[t])
            # self.deltaW += delta_out @ s[t][None, :]
            # delta_in = self.W.T @ delta_out @ ((s[t] * (1 - s[t]))[:, None])
            # self.deltaV += delta_in @ (np.eye(self.vocab_size)[x[t]][None, :])
            # self.deltaU += delta_in @ s[t - 1][None, :]
            d_oh, x_oh = np.zeros((self.out_vocab_size, 1)), np.zeros((self.vocab_size, 1))
            d_oh[d[t]] = 1
            x_oh[x[t]] = 1
    ...
    def compute_mean_loss(self, X, D):
        ...
        for ind in range(len(X)):
            mean_loss += self.compute_loss(X[ind], D[ind])
            tot_words += len(X[ind])
        mean_loss /= tot_words
        return mean_loss
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        print(f'full dev loss: {full_dev_loss}')
        print(f'full dev adj loss: {full_dev_adj_loss}')
