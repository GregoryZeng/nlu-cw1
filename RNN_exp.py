...
class RNN(object):
    ...
    def compute_loss(self, x, d):
        ...
        return - np.sum(d_onehot * np.log(y_hat[0]))
    ...
    def compute_loss_np(self, x, d):
        ...
        return - np.sum(d_onehot * np.log(y_hat[0][-1]))
    ...
    def compute_mean_loss_np(self, X, D):
        ...
        for i in range(len(X)):
            total_loss += self.compute_loss_np(X[i], D[i])
            n_words += len(X[i])
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        best_loss = my_rnn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback, epochs=10)
        ...
        mean_loss = my_rnn.compute_mean_loss(X_test, D_test)
        adjusted_loss = adjust_loss(mean_loss, fraction_lost, q)
        print("Mean loss: {}".format(mean_loss))
        print("Unadjusted: %.03f" % np.exp(mean_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
    ...
    if mode == "exp2":
        """  words from the subject index (inclusive)until the verb index """
        ...
        np.save(out_folder + "/rnn.U_h{}_s{}_r{}_e{}.npy".format(hdim,lookback,lr, epochs), my_rnn.U)
        np.save(out_folder + "/rnn.V_h{}_s{}_r{}_e{}.npy".format(hdim,lookback,lr, epochs), my_rnn.V)
        np.save(out_folder + "/rnn.W_h{}_s{}_r{}_e{}.npy".format(hdim,lookback,lr, epochs), my_rnn.W)
    ...
    if mode == "exp3":
        """ at least 5 words between subject and verb"""
    ...
    if mode == 'exp_pred':
        U = np.load('exp3/rnn.U_h50_s1_r4.0_e25.npy')
        V = np.load('exp3/rnn.V_h50_s1_r4.0_e25.npy')
        W = np.load('exp3/rnn.W_h50_s1_r4.0_e25.npy')
        vocab_size = len(V[0])
        hdim = len(U[0])
        ...
        np.save('hidden_C.npy', np.array(a_hidd))

    if mode == 'exp-test':
        U = np.load('exp1/rnn.U_h5_s1_r2.0_e25.npy')
        V = np.load('exp1/rnn.V_h5_s1_r2.0_e25.npy')
        W = np.load('exp1/rnn.W_h5_s1_r2.0_e25.npy')
        vocab_size = len(V[0])
        hdim = len(U[0])
        ...
        mean_loss = r.compute_mean_loss_np(X_test, D_test)
        adjusted_loss = adjust_loss(mean_loss, fraction_lost, q)
        np_acc_test = sum([
            r.compute_acc_np(X_test[i], D_test[i])
            for i in range(len(X_test))
        ]) / len(X_test)
        print("Accuracy: {}".format(np_acc_test))
        print("Mean loss: {}".format(mean_loss))
        print("Unadjusted: %.03f" % np.exp(mean_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
